...
def update_chart(relayout_data):
    '''优先计算最近7天范围'''
    end_ms = pd.read_parquet('soldata.parquet', columns=['ts'])['ts'].iloc[-1]
    start_ms = pd.read_parquet('soldata.parquet', columns=['ts'])['ts'].iloc[0]
    ...
    '''在此处添加订单买卖点'''
    orders = pd.read_parquet('测试集.parquet')
    ...
    '''添加净值曲线变化图'''
    trade_df = pd.read_parquet('交易记录.parquet')
