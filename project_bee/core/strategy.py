...
class Strategy():
    ...
    def plotlastbacktest(self, lastfilename):
        print(f'\n=================== 启动回测可视化 ===================')
        price_file_path = os.path.join('datas', lastfilename)
        orders_file_path = os.path.join('datas', 'ordersplotting.parquet')
        trades_file_path = os.path.join('datas', 'tradesplotting.parquet')
        plot_backtest(
    ...
    def runparamscanner(self , cpunum , paramlist:list=[] , tokens:list=['btcdata.parquet']):
        ...
        if results:
            ...
            print(rdf.head(30))
            # 保存结果
            rdf.to_csv(f'回测扫描结结果/{self.name}扫描结果.csv', index=False)
            print(f"\n✅ 结果已保存至 回测扫描结结果/{self.name}扫描结果.csv")
        else:
