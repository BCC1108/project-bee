...
import os
from config import okx_api_key , okx_api_secret_key , okx_passphrase , flag

# ====== API Key 设置 ======
apiKey = okx_api_key
apiSecretKey = okx_api_secret_key
passphrase = okx_passphrase
flag = flag

# 检查API密钥是否配置
if not apiKey or not apiSecretKey or not passphrase:
    print("错误: 请配置OKX API密钥环境变量")
    print("请在 .env 文件中设置 OKX_API_KEY, OKX_API_SECRET, OKX_PASSPHRASE")
    exit(1)
...
def ordnumcauculating(money):
    ...
    #ordnum = min((money * int(lever) / price_now / swapCcy) , 3000)
...
def getKlines(instId, ckp ):
    ...
    try:
        ...
        low_oc = min(df['open'].iloc[-1], df['close'].iloc[-1])
        high_oc = max(df['open'].iloc[-1], df['close'].iloc[-1])
        boll = df['bollmid'].iloc[-1]
        condition = (boll >= low_oc) and (boll <= high_oc)
...
# 主函数
def main():
    global instId , barperiod , cash , lever , swapCcy , swapMin ,  qty , cycleNum , real_time_equility , last_time_update
    
    #命令行参数，如果不想命令行输入，不如试着修改default条目然后直接run即可
    parser = argparse.ArgumentParser(description="hqbt2交易策略")
    parser.add_argument("--symbol", "-s",     type = str,   required = False,  default = 'SOL-USDT-SWAP',   help = "交易对, eg: BTC-USDT-SWAP ")
    parser.add_argument("--barperiod", "-b",  type = str,   required = False,  default = '1H',              help = "k线的bar , 如 1s 1m 3m 5m 15m .....")
    ...
    parser.add_argument("--tpr", "-tr",       type = float, required = False,  default = 0.15,              help = "止盈比例")
    parser.add_argument("--slr", "-sr",       type = float, required = False,  default = 0.40,              help = "止损比例")
