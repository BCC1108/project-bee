...
location = 'database/jlmemory'
memory = Memory(location , verbose=1)

# 确保 database/rowdatas 文件夹存在
data_dir = 'database/rowdatas'
...
def make_df(datafilename):
    ...
    data_dir = 'database/rowdatas'  # 和保存时一致
...
def convert_to_parquet(filenames):
    ...
    for file in filenames:
        ...
        print(df.head(20))
        file_path = os.path.join('database/rowdatas', f'{file}.parquet')
